...
mainWorker args = topHandler $
  ...
  where
     ...
     printErrors errs = SE.die $ concat (intersperse "\n" errs)
...
importAction (CabalFlags pkgdb prefix setup) s g = do
  when (null s) $ SE.die "at least import directory has to be specified"
  let dir = head s
  cd <- getCurrentDirectory
  doesDirectoryExist dir >>= return . not >>= flip when (SE.die $ "import directory " ++ 
                                                                dir ++ " does not exist")
...
cloneAction f s g = do
  when (null s) $ SE.die "at least one package name to clone should be specified"
  mapM clonePackage s
  return ()
...
bootstrapAction f s g = do
  ...
  let exfail (ExitFailure _) = True
      exfail _ = False
  when (exfail ex) $ SE.die "ghc-pkg failed to initialize package configuration:\n"
